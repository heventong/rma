	...
	<form action="{{url('update')}}" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data" >
        ...
        <div class="row cl">
            ...
            <div>
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" name="banner_title" class="input-text" value="{{$Data['banner_title']}}">
                    <input type="text" name="banner_title_ch" class="input-text" value="{{$Data['banner_title_ch']}}">
                    <input type="text" name="banner_title2" class="input-text" value="{{$Data['banner_title2']}}">
                    <input type="text" name="banner_title2_ch" class="input-text" value="{{$Data['banner_title2_ch']}}">
                    <input type="text" name="title" class="input-text" value="{{$Data['title']}}">
                    <textarea style="width:100%;height:100px" type="text" name="content" class="input-text" value="{{$Data['content']}}">{{$Data['content']}}</textarea>
                    <textarea style="width:100%;height:100px" type="text" name="content_ch" class="input-text" value="{{$Data['content_ch']}}">{{$Data['content_ch']}}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="row cl">
            ...
            <div  style="overflow: hidden">
                <div class="btn btn-secondary" onclick='upload(this,"news",1)'>修改</div>
                <br/>
                <div class="info">
                    <div class="pic-box">
                        <<?php echo (substr($Data['news']['pic'],strrpos($Data['news']['pic'],'.')+1)=='mp4'?'video':'img');?> style="width:100%"  src="{{asset($Data['news']['pic'])}}">
                        <p class="state">视频</p>
                            <input  name="news[pic]"  value="{{$Data['news']['pic']}}">
        ...
        <label class="form-label col-xs-4 col-sm-2">编辑：</label>
        <div  style="overflow: hidden">
        <div class="btn btn-secondary" onclick='upload(this,"news2",1)'>修改</div>
        <br/>
        <div class="info">
        <div class="pic-box"><<?php echo substr($Data['news2']['pic'],strrpos($Data['news2']['pic'],'.')+1)=='mp4'?'video':'img';?> style="width:100%"  src="{{asset($Data['news2']['pic'])}}">
        <p class="state"></p>
        <input type="hidden"  name="news2[pic]"  value="{{$Data['news2']['pic']}}">
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
                ...
                <?php if($_GET['edit']=='news'){?>
                <a target="_blank"  href="{{url('news')}}">打开当前编辑的页面</a> <?php }else{?>
                <a target="_blank"  href="{{url('newAdd?new_page=pc&new_page_name='.$_GET['edit'])}}">打开当前编辑的页面</a> <?php }?>
			</div>
		</div>
    ...
    function set_upload(upload_value,type,count=0) {
        ...
        if(count == 1){
            var contact = "<div class='pic-box upload_pic'><input name='"+upload_value.name+"[pic]' type='hidden' value='"+upload_value.value+"'>" +
                "<img style='width: 100px;max-height: 100px' name='"+upload_value.name+"' src='"+upload_value.value+"' value='"+upload_value.value+"'/></div>";
            result+=contact;
            return result;
        }
