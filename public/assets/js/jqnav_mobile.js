...
function firstScreen() {
    ...
    /*back to work */
    if (sW <= 770) {
        $(".work-nav").css({ "margin-top": "48px", "background": "#fff" });
    } else {
    ...
    if (sW <= 560) {
        $(".detail-randoms .random").css({ "height": sW });
    } else if (sW > 560 && sW <= 950) {
        $(".detail-randoms .random").css({ "height": sW / 3 });
    } else {
        $(".detail-randoms .random").css({ "height": 950 / 3 });
    }
...
function careers() {
    $(".careers-lists li h3 .btn-view").on("click", function() {
        ...
        if (view.hasClass("cView")) {
            ...
            $(this).html("View <i class='icon-add'></i>");
        } else {
            view.addClass("cView").next(".careers-text").slideDown();
            $(this).html("Close <i class='icon-minus'></i>");
        }
    });
}
