...
<section class="Hui-article-box">
	...
	<form action="{{url('update')}}" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data" >
        ...
        <?php if($_GET['edit']=='xclub'){?>
        <input type="hidden" name="page_name" value="workdetailxclub" />
        <?php }else{?>
        ...
        <div class="row cl">
                ...
                <div  style="overflow: hidden">
                    ...
                    <div class="info">
                        ...
                        <div class="pic-box"><img  src="{{asset($value['pic'])}}">
                            ...
                            <p>title_desc:<input type="text" name="banner[{{$key}}][title_desc]" value="{{$value['title_desc']}}"/></p>
                            <span>链接</span>
            ...
            <div>
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <p><input type="text" name="page_content_name" value="{{$Data['page_content_name']}}"></p>
                    <p><input type="text" name="service" value="{{$Data['service']}}"></p>
                    <p><input type="text" name="service_zh" value="{{$Data['service_zh']}}"></p>
                    <textarea style="width:800px;height:100px" type="text" name="page_content" class="input-text" >{{$Data['page_content']}}</textarea>
        ...
        <div class="row cl">
            ...
            <label class="form-label col-xs-4 col-sm-2">编辑：</label>
            <div  style="overflow: hidden">
                <div class="btn btn-secondary" onclick='upload(this,"workdetail6",1,1)'>修改</div>
                <br/>
                <div class="info">
                    <div class="pic-box"><img  src="{{asset($Data['workdetail6'])}}">
                        <p class="state">原图</p>
                        <input type="hidden" name="workdetail6" value="{{$Data['workdetail6']}}">
                    </div>
                </div>
            </div>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
                ...
                <?php if($_GET['edit']=='xclub'){?>
                <a target="_blank"  href="{{url('workDetail?project=xclub')}}">打开当前编辑的页面</a>
                <?php }else{?>
...
<script>
    ...
    function set_upload(upload_value,type,count=0,link_type=0) {
        ...
        $.ajax({
            url:'{{url('getlang')}}',
            data:{'lang_name':lang+'.php','type':'mobile'},
            async: false,
            success:function (data) {
                tmp=JSON.parse(data);
            }
        ...
        for(var i in upload_value){
            var contact = "<div class='pic-box upload_pic'><input name='"+upload_value[i].name+"["+i+"][pic]' type='hidden' value='"+upload_value[i].value+"'>" +
                ...
                "<p>sub_title:<input type='text' name='"+upload_value[i].name+"["+i+"][sub_title]' value='"+(arrs_length > i?tmp['data'][type][i]['sub_title']:tmp['data'][type][0]['sub_title'])+"'></p>"+
                    "<p>title_desc:<input type='text' name='"+upload_value[i].name+"["+i+"][title_desc]' value='"+(arrs_length > i?tmp['data'][type][i]['title_desc']:tmp['data'][type][0]['title_desc'])+"'></p>";
